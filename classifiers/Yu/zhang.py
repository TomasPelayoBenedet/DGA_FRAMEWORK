...
from tensorflow.keras.layers import Dense, Embedding, ThresholdedReLU, Conv1D, MaxPooling1D, Flatten, Dropout
...
class Zhang(Classifier):
    ...
    def __init__(self) -> None:
        ...
        self.model.add(Conv1D(filters=128, kernel_size=3, padding = 'same',
                              strides=1))
        self.model.add(ThresholdedReLU(1e-6))
        self.model.add(MaxPooling1D(pool_size=2, padding='same'))
        self.model.add(Conv1D(filters=128, kernel_size=2, padding='same', strides=1))
        self.model.add(ThresholdedReLU(1e-6))
        self.model.add(MaxPooling1D(pool_size=2, padding='same'))
